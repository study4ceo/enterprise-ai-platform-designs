"""
QLoRA Trainer - 4-bit quantized LoRA training for memory efficiency
"""
...
class QLoRATrainer:
    ...
    Implements 4-bit quantized parameter-efficient fine-tuning.
    Uses 50% less memory than standard LoRA while maintaining quality.
    """
    ...
    def __init__(self, config: QLoRATrainingConfig):
        ...
        if self.device != "cuda":
            raise RuntimeError("QLoRA requires CUDA/GPU. CPU training not supported.")
        
        self.model = None
        self.tokenizer = None
        self.trainer = None
        
        logger.info(f"QLoRATrainer initialized. Device: {self.device}")
    ...
    def load_model(self) -> None:
        ...
        logger.info(f"Loading base model with 4-bit quantization from {self.config.model_path}")
        ...
        try:
            ...
            logger.info(f"4-bit quantization config: {bnb_config}")
            ...
            logger.info(f"Memory saved: ~50% compared to full precision LoRA")
        ...
        try:
            ...
            def tokenize_function(examples):
                ...
                if 'text' in examples:
                    ...
                    raise ValueError("Dataset must have 'text', 'prompt'+'completion', or 'instruction'+'output' columns")
    ...
    def train(
        ...
        """
        Train the model with QLoRA (4-bit quantized LoRA).
        ...
        try:
            ...
            training_args = TrainingArguments(
                ...
                max_grad_norm=0.3,  # Important for stability with quantization
            ...
            if self.device == "cuda":
                ...
                metrics["final_gpu_memory_gb"] = final_mem
                logger.info(f"Peak GPU memory: {peak_mem:.2f} GB")
                logger.info(f"Final GPU memory: {final_mem:.2f} GB")
    ...
    def save_checkpoint(self, checkpoint_dir: str) -> str:
        """
        Save training checkpoint (LoRA adapters only).
        ...
        logger.info(f"QLoRA checkpoint saved to {checkpoint_dir}")
        logger.info("Note: Only LoRA adapters saved. Base model needs to be loaded separately.")
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        if self.device == "cuda":
            return {
                ...
                "quantization": "4-bit",
                "memory_savings": "~50% vs full precision"
            }
        return {}
