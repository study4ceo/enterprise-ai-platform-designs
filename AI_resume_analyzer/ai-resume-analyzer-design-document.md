...
| Technology | Version | Why This Choice |
|------------|---------|-----------------|
| **Golang** | 1.22+ | High-performance API services, concurrent file processing |
| **Python** | 3.12+ | AI/ML pipelines, document parsing, NLP tasks |
| **Fiber** | 2.x | Fast web framework for Go (Express-like API) |
| **FastAPI** | 0.111+ | Python async API framework for AI services |
| **PostgreSQL** | 16.x | Advanced JSON support, performance improvements |
| **Redis** | 7.x | Caching, session storage, queue management |
| **gRPC** | Latest | Inter-service communication (Go ↔ Python) |
...
| WebSocket Server | Golang | Fiber WebSocket | Real-time updates, streaming |
| File Parser | Python | FastAPI | PDF/DOCX parsing, text extraction |
| AI Analyzer | Python | LangChain | Resume analysis, skill extraction |
| Job Matcher | Golang | Native | Vector search, matching algorithm |
| Background Worker | Golang | Asynq | Async job processing |

**Inter-Service Communication:**
- **WebSockets** for real-time client updates (upload progress, analysis streaming)
...
### **Testing Stack (TDD-First Approach)**

| Technology | Version | Purpose |
|------------|---------|---------|
| **Go Testing** | Built-in | Unit tests for Go services |
| **Testify** | 1.9+ | Assertion library for Go |
| **PyTest** | 8.x | Unit & integration tests for Python |
| **Hypothesis** | 6.x | Property-based testing |
| **Vitest** | 2.x | Frontend unit tests |
| **Playwright** | 1.45+ | End-to-end testing |
| **Testcontainers** | Latest | Integration tests with real DBs |
...
### **Architecture Pattern**: Microservices with Go + Python Backend
...
└────────────────────┬────────────────────────────────────────┘
       ...
       REST API          WebSocket (wss://)
            │                 │
┌───────────▼─────────────────▼───────────────────────────────┐
│         API Gateway + WebSocket Server (Golang + Fiber)     │
│    Auth | Rate Limiting | Request Routing | Logging        │
│    Real-time Progress | Streaming AI Responses             │
└──────┬──────────────┬──────────────┬────────────────────────┘
       │              │              │
       │    gRPC      │    gRPC      │    gRPC
       │              │              │
...
**Flow with Real-Time Updates:**
1. User uploads resume (PDF/DOCX/TXT)
2. **WebSocket connection established**
3. File stored in Vercel Blob → **WS: "upload_progress: 100%"**
4. Background job triggered (Inngest) → **WS: "parsing_started"**
5. Text extraction using appropriate parser → **WS: "parsing_progress: 50%"**
6. Store raw text + metadata in PostgreSQL → **WS: "parsing_complete"**
7. Trigger AI analysis → **WS: "analysis_started"**
8. Stream AI analysis results → **WS: "analysis_chunk: {...}"**

**WebSocket Implementation (Golang + Fiber):**
...
func handleWebSocket(c *websocket.Conn) {
    ...
    // Register connection
    manager.connections[sessionID] = c
    defer func() {
...
func handleBroadcast() {
    for msg := range manager.broadcast {
        if conn, ok := manager.connections[msg.SessionID]; ok {
            if err := conn.WriteJSON(msg); err != nil {
                log.Println("Broadcast error:", err)
                conn.Close()
                delete(manager.connections, msg.SessionID)
            }
        }
    }
}
...
**Frontend WebSocket Client (React):**

```typescript
// hooks/useResumeWebSocket.ts
...
export const useResumeWebSocket = (sessionId: string) => {
  const [progress, setProgress] = useState<WSProgressEvent[]>([]);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      const message: WSProgressEvent = JSON.parse(event.data);
      ...
      switch (message.type) {
        case 'upload_progress':
          console.log('Upload progress:', message.data.percentage);
          break;
        case 'parsing_started':
          console.log('Parsing started');
          break;
        case 'parsing_progress':
          console.log('Parsing progress:', message.data.percentage);
          break;
        ...
        case 'analysis_started':
          ...
          break;
        case 'analysis_chunk':
          console.log('AI chunk received:', message.data);
          break;
  ...
  return { progress, status, sendMessage };
};
```
...
export const ResumeUploader = () => {
  ...
  const { progress, status, sendMessage } = useResumeWebSocket(sessionId || '');
  ...
  const handleUpload = async (file: File) => {
    ...
    // Upload file via REST API (or tus protocol for chunked upload)
    ...
    // WebSocket will handle all progress updates
  };
    ...
    <div className="space-y-4">
      ...
      {progress.length > 0 && (
        <div className="space-y-2">
          ...
          {latestProgress?.type === 'analysis_started' && (
...
function calculateProgress(events: WSProgressEvent[]): number {
  const stages = {
    upload_progress: 20,
    parsing_started: 30,
    parsing_progress: 50,
  ...
  return stages[latest?.type as keyof typeof stages] || 0;
}
...
**Tech Stack:**
- GPT-5.5 for structured extraction (streaming mode)
- Claude Opus 4.8 for detailed analysis (streaming mode)
...
```python
# ai_service.py - Python FastAPI with streaming
...
class WebSocketCallback(StreamingStdOutCallbackHandler):
    """Custom callback to stream AI responses via WebSocket"""
...
async def websocket_analyze(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        llm = ChatOpenAI(
            model="gpt-5.5",
            streaming=True,
            callbacks=[WebSocketCallback(websocket, session_id)]
        )
...
export const useStreamingAnalysis = (sessionId: string, resumeText: string) => {
  const [chunks, setChunks] = useState<string[]>([]);
  const [isComplete, setIsComplete] = useState(false);
  const [error, setError] = useState<string | null>(null);
  
  useEffect(() => {
    const ws = new WebSocket(`wss://ai-service.com/ws/analyze/${sessionId}`);
    
    ws.onopen = () => {
      // Send resume data
      ws.send(JSON.stringify({ resume_text: resumeText }));
    };
    ...
    ws.onmessage = (event) => {
      ...
      switch (message.type) {
        ...
        case 'analysis_chunk':
          setChunks(prev => [...prev, message.data.token]);
          break;
